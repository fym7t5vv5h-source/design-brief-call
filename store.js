import { config, proxiedAssetUrl } from "./config.js";
...
function friendlyNetworkError(err, fallback) {
  ...
  if (/load failed|failed to fetch|networkerror|network error|connection|reset|timeout|вход не ответил|не удалось загрузить/i.test(msg)) {
    return new Error(
      "Нет связи с облаком. Сайт откройте на design-brief-ecru.vercel.app (после обновления вход без VPN). Или включите VPN и обновите страницу."
    );
  }
  return new Error(fallback || msg || "Ошибка сети");
}
...
export async function getSupabase() {
  ...
  try {
    ...
    throw friendlyNetworkError(err, "Сеть: не удалось загрузить вход");
  }
}
...
export async function getCloudSession() {
  try {
    ...
    if (!sb) return null;
    const { data, error } = await withTimeout(sb.auth.getSession(), 12000, "Сеть: вход не ответил");
    if (error) throw error;
    const session = data.session;
    if (!session?.user?.id || session.user.id === "local") return null;
    return session;
  } catch (err) {
    throw friendlyNetworkError(err, "Сеть: вход не ответил");
  }
}
...
export async function signInOrRegister(email, password) {
  ...
  if (/load failed|failed to fetch|network/i.test(loginMsg)) {
    throw friendlyNetworkError(login.error);
  }
  ...
  try {
    ...
    throw friendlyNetworkError(err, "Сеть: регистрация не ответила");
  }
  ...
  if (created.error) {
    ...
    throw friendlyNetworkError(created.error, msg);
  }
...
export async function deleteProject(id) {
  if (await useLocal()) return localApi.deleteProject(id);
  const sb = await getSupabase();
...
export async function uploadImage(briefId, file) {
  ...
  return proxiedAssetUrl(data.publicUrl) || data.publicUrl;
}
